...
current_users = ['Adobo', 'CreamyCrop', 'iCollect', 'sim0nsays', 
                 'sideways2piece', 'greenDuckee']

new_users = ['BlowingHard', 'sideways2Piece', 'creamycrop', 
             'oRAnGEhAIr', 'KazaKhan', 'FloatMeByeee']

# convert current_users to lowercase for case-insensitve comparison
current_users_lower = [user.lower() for user in current_users]

for new_user in new_users:
    if new_user.lower() in current_users_lower:
        print(f"Username {new_user} is not available to use. Please enter a new username.")
    else:
